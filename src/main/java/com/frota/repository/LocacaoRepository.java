...
public class LocacaoRepository {
    ...
    private final String BASE_SELECT = 
        ...
        "up.nome as proprietario_nome, up.email as proprietario_email, up.telefone as proprietario_telefone, " +
        "p.chave_pix as proprietario_chave_pix " +
        "FROM locacoes l " +
    ...
    public List<Locacao> listarPorCliente(Long clienteId) {
        List<Locacao> l = new ArrayList<>();
        String sql = BASE_SELECT + "WHERE l.cliente_id = ? ORDER BY l.created_at DESC";
    ...
    public List<Locacao> listarPorProprietario(Long proprietarioId) {
        List<Locacao> l = new ArrayList<>();
        String sql = BASE_SELECT + "WHERE m.proprietario_id = ? ORDER BY l.created_at DESC";
        try (Connection c = Conexao.getConnection(); PreparedStatement p = c.prepareStatement(sql)) {
    ...
    public List<Locacao> listarAtivas() {
        List<Locacao> l = new ArrayList<>();
        String sql = BASE_SELECT + "WHERE l.status IN ('PENDENTE','ATIVA','ATRASADA') ORDER BY l.data_inicio";
        try (Connection c = Conexao.getConnection(); Statement s = c.createStatement(); ResultSet r = s.executeQuery(sql)) {
            while (r.next()) l.add(mapear(r));
        } catch (SQLException e) { throw new RuntimeException(e); }
        return l;
    ...
    public List<Locacao> listarRecentes(int limite) {
        List<Locacao> l = new ArrayList<>();
        String sql = BASE_SELECT + "ORDER BY l.created_at DESC LIMIT ?";
        try (Connection c = Conexao.getConnection(); PreparedStatement p = c.prepareStatement(sql)) {
            p.setInt(1, limite);
            try (ResultSet r = p.executeQuery()) { while (r.next()) l.add(mapear(r)); }
        } catch (SQLException e) { throw new RuntimeException(e); }
    ...
    public java.math.BigDecimal calcularFaturamentoProprietario(Long proprietarioId) {
        String sql = "SELECT SUM(l.valor_total) FROM locacoes l JOIN maquinas m ON l.maquina_id = m.id WHERE m.proprietario_id = ? AND l.status IN ('ATIVA', 'CONCLUIDA')";
        try (Connection c = Conexao.getConnection(); PreparedStatement p = c.prepareStatement(sql)) {
            p.setLong(1, proprietarioId);
            try (ResultSet r = p.executeQuery()) {
                if (r.next() && r.getBigDecimal(1) != null) {
    ...
    private Locacao mapear(ResultSet r) throws SQLException {
        ...
        try {
            l.setProprietarioNome(r.getString("proprietario_nome"));
            l.setProprietarioEmail(r.getString("proprietario_email"));
            l.setProprietarioTelefone(r.getString("proprietario_telefone"));
            l.setProprietarioChavePix(r.getString("proprietario_chave_pix"));
